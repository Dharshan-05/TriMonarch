...
import { Partner, UpdatePartnerInput, PartnerType, PartnerStatus } from '@/services/partners.service';
...
interface EditPartnerModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdatePartnerInput) => Promise<void>;
  isLoading?: boolean;
}
...
export const EditPartnerModal: React.FC<EditPartnerModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(errorObj?.response?.data?.error?.message || errorObj?.message || 'Failed to update partner');
    }
  };
  ...
  return (
    <Dialog
      ...
      description="Update business partner details, contact channels, or active status."
      className="max-w-md"
      footer={
        <>

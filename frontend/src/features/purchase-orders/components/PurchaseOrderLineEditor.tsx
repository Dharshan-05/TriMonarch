...
interface PurchaseOrderLineEditorProps {
  ...
  onLinesChange: (lines: DraftPurchaseLine[]) => void;
  currency?: string;
}

export const PurchaseOrderLineEditor: React.FC<PurchaseOrderLineEditorProps> = ({
  lines,
  onLinesChange,
  currency = 'USD',
}) => {
  ...
  const handleAddLine = () => {
    const newLine: DraftPurchaseLine = {
  ...
  const handleUpdateLine = (index: number, updatedFields: Partial<DraftPurchaseLine>) => {
    const updated = [...lines];
    ...
    if (updatedFields.product_id) {
      ...
      if (selectedProd) {
        ...
        if (selectedProd.cost) {
          current.unit_cost = selectedProd.cost;
        } else if (selectedProd.price) {
          current.unit_cost = selectedProd.price;
        }
      }
    }

    const qtyNum = parseFloat(current.quantity || '0');
    const costNum = parseFloat(current.unit_cost || '0');
    const discountNum = parseFloat(current.discount_amount || '0');
    const taxRateNum = parseFloat(current.tax_rate || '0');

    const subtotal = Math.max(0, qtyNum * costNum - discountNum);
  ...
  lines.forEach((l) => {
    ...
    const costNum = parseFloat(l.unit_cost || '0');
    ...
    <div className="space-y-3 border p-3 rounded-lg bg-card">
      ...
      {lines.length === 0 ? (
        <div className="text-center py-6 border border-dashed rounded text-xs text-muted-foreground">
          No items added. Click &quot;Add Product Line&quot; to build purchase order lines.
        </div>
      ) : (
              ...
              <div className="col-span-4 sm:col-span-2">
                <FormField label="Unit Cost" required>
                  <Input
                    ...
                    onChange={(e) => handleUpdateLine(idx, { unit_cost: e.target.value })}
                    placeholder="0.0000"
                    className="h-8 font-mono text-xs"
                  />
                </FormField>

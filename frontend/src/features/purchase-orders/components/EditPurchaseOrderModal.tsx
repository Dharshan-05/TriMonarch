...
import { PurchaseOrder, UpdatePurchaseOrderInput } from '@/services/purchaseOrders.service';

interface EditPurchaseOrderModalProps {
  order: PurchaseOrder | null;
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdatePurchaseOrderInput) => Promise<void>;
  isLoading?: boolean;
}
...
export const EditPurchaseOrderModal: React.FC<EditPurchaseOrderModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(errorObj?.response?.data?.error?.message || errorObj?.message || 'Failed to update purchase order');
    }
  };
  ...
  return (
    <Dialog
      ...
      description="Update draft procurement parameters before submission or approval."
      className="max-w-md"
      footer={
        <>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="p-3 border rounded-lg bg-surface space-y-1 text-xs">
          <div className="flex justify-between text-muted-foreground">
            ...
            <span className="font-semibold text-foreground">{order.supplier_name || order.supplier_id}</span>
          </div>
          <div className="flex justify-between text-muted-foreground">

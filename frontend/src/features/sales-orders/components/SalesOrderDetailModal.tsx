...
export const SalesOrderDetailModal: React.FC<SalesOrderDetailModalProps> = ({
  ...
  return (
    <Dialog
      ...
      title="Sales Order Summary Profile"
      description="Detailed breakdown of sales order lines, customer assignment, and financial totals."
      className="max-w-2xl"
      footer={
    ...
    >
      <div className="space-y-4 py-2 text-xs max-h-[75vh] overflow-y-auto pr-1">
        ...
        {/* Customer & Date Context */}
        <div className="grid grid-cols-2 gap-3 border p-3 rounded-lg bg-card">
          <div>
            <span className="text-[11px] text-muted-foreground block">Customer Reference</span>
        ...
        <div className="space-y-1.5">
          <h4 className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
            Order Line Breakdown ({items.length})
          </h4>
          {items.length === 0 ? (
                ...
                <TableBody>
                  {items.map((item, idx) => (
                    <TableRow key={item.id || idx}>
                      ...
                      <TableCell align="right" className="font-mono text-xs tabular-nums">
                        {formatCurrency(parseFloat(item.unit_price || '0'), order.currency)}
                      </TableCell>
        ...
        <div className="flex justify-between text-[10px] text-muted-foreground font-mono border-t pt-2">
          <span>Created: {order.created_at ? formatDate(order.created_at) : 'N/A'}</span>
          <span>Last Modified: {order.updated_at ? formatDate(order.updated_at) : 'N/A'}</span>
        </div>
      </div>
    </Dialog>
  );

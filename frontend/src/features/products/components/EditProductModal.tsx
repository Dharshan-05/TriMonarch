...
import { Product, UpdateProductInput, ProductStatus } from '@/services/products.service';
import {
...
interface EditProductModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (id: string, data: UpdateProductInput) => Promise<void>;
  isLoading?: boolean;
}
...
export const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  useEffect(() => {
    if (product) {
      ...
      setPrice(product.price || '0.0000');
      setCost(product.cost || '0.0000');
      setTaxRate(product.tax_rate || '0.000000');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(errorObj?.response?.data?.error?.message || errorObj?.message || 'Failed to update product');
    }
  };
  ...
  return (
    <Dialog
      ...
      description="Update SKU, pricing, category, or status for this product master entry."
      className="max-w-md"
      footer={
        <>
